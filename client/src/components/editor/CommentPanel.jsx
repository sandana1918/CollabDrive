...
export const CommentPanel = ({ open, comments = [], canComment, selectedText, onAddComment, onResolveComment }) => {
  ...
  return (
    <aside className="w-full rounded-[24px] border border-[#e0e3e7] bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.08)] xl:w-[320px]">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#5f6368]">Comments</p>
          <h3 className="mt-1 text-lg font-medium text-[#202124]">Discussion</h3>
        </div>
        <span className="rounded-full bg-[#f1f3f4] px-3 py-1 text-xs font-medium text-[#5f6368]">{unresolved.length} open</span>
      </div>

      {canComment ? (
        <div className="mt-4 rounded-[18px] border border-[#e6ebf2] bg-[#fafcff] p-3">
          <p className="text-xs text-[#5f6368]">Selection</p>
          ...
          <textarea
            ...
            className="mt-3 min-h-[96px] w-full resize-none rounded-2xl border border-[#d7dce5] bg-white px-4 py-3 text-sm text-[#202124] outline-none focus:border-[#8ab4f8]"
          />
          ...
          <div key={comment.id} className={`rounded-[18px] border px-4 py-3 ${comment.resolved ? "border-[#e9edf3] bg-[#f8fafc] opacity-70" : "border-[#e0e7f1] bg-white"}`}>
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-[#5f6368]">{new Date(comment.createdAt).toLocaleString()}</p>
              </div>
            ...
            {comment.anchorText ? <p className="mt-3 rounded-xl bg-[#f5f8fd] px-3 py-2 text-xs text-[#5f6368]">“{comment.anchorText}”</p> : null}
            <p className="mt-3 text-sm leading-6 text-[#202124]">{comment.message}</p>
          </div>
        )) : <p className="text-sm text-[#5f6368]">No comments yet.</p>}
      </div>
    </aside>
  );
};
