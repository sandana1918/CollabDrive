...
export const SlashMenu = ({ open, query = "", onSelect }) => {
  ...
  return (
    <div className="absolute left-0 top-[calc(100%+10px)] z-30 min-w-[220px] rounded-[20px] border border-[#dde5f0] bg-white p-2 shadow-[0_18px_40px_rgba(15,23,42,0.14)]">
      ...
      {filtered.map((item) => (
        <button key={item.key} type="button" className="block w-full rounded-xl px-3 py-2 text-left text-sm text-[#202124] hover:bg-[#fbf6f9]" onMouseDown={(event) => event.preventDefault()} onClick={() => onSelect(item.key)}>
          {item.label}
        </button>
      ))}
    </div>
