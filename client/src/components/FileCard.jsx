...
const ActionsMenu = ({ file, canManage, onOpen, onShare, onDelete, onStar, onPin, onRestore }) => {
  ...
  return (
    <div className="relative">
      ...
      {open ? (
        <div className="absolute right-0 top-[calc(100%+8px)] z-20 min-w-[180px] rounded-2xl border border-[#e1e8f2] bg-white p-2 shadow-card">
          ...
          {file.category !== "folder" ? <a className="block w-full rounded-xl px-3 py-2 text-left text-sm text-drive-text transition hover:bg-slate-50" href={filesApi.downloadUrl(file.id)} target="_blank" rel="noreferrer" onClick={() => setOpen(false)}>Download</a> : null}
    ...
    <div className="group overflow-hidden rounded-2xl border border-[#e0e6ef] bg-white transition duration-200 hover:border-[#c7d7f2] hover:shadow-soft">
      <div className="flex items-center justify-between border-b border-[#edf2f8] px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className={`grid ${denseMode ? "h-9 w-9" : "h-10 w-10"} place-items-center rounded-2xl ${isDocument ? "bg-[#e8f0fe] text-drive-blue" : isFolder ? "bg-[#fff6e5] text-[#f29900]" : "bg-[#f1f5f9] text-slate-600"}`}>
      ...
      <div className={`px-4 ${denseMode ? "py-3" : "py-4"}`}>
        <div className={`rounded-2xl ${isDocument ? "bg-[#f8fbff]" : "bg-[#fafbfc]"} p-4`}>
          {file.preview?.kind === "image" ? <img src={downloadUrl} alt={file.filename} className="h-28 w-full rounded-xl object-cover" /> : null}
          {file.preview?.kind === "pdf" ? <iframe src={downloadUrl} title={file.filename} className="h-32 w-full rounded-xl border border-[#e6ebf3] bg-white" /> : null}
          {file.preview?.kind === "none" ? <p className="min-h-[60px] text-sm leading-6 text-[#475467] prose-preview">{isDocument ? file.contentPreview || "Open and edit with your collaborators in real time." : isFolder ? "Use folders to create a nested Drive-like hierarchy." : "Stored securely and ready for quick download or sharing."}</p> : null}
        </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          <button className="rounded-full bg-drive-blueSoft px-4 py-2 text-sm font-medium text-[#174ea6] transition hover:bg-[#dbe7ff]" onClick={() => onOpen(file)}>Open</button>
          <button className="grid h-9 w-9 place-items-center rounded-full text-drive-subtext transition hover:bg-slate-100 hover:text-drive-text" onClick={() => onStar(file)}>{file.isStarred ? <StarSolidIcon className="h-5 w-5 text-[#fbbc04]" /> : <StarIcon className="h-5 w-5" />}</button>
          {!isFolder ? <a className="grid h-9 w-9 place-items-center rounded-full text-drive-subtext transition hover:bg-slate-100 hover:text-drive-text" href={downloadUrl} target="_blank" rel="noreferrer"><ArrowDownTrayIcon className="h-5 w-5" /></a> : null}
          {canManage && !file.isTrashed ? <button className="grid h-9 w-9 place-items-center rounded-full text-drive-subtext transition hover:bg-slate-100 hover:text-drive-blue" onClick={() => onShare(file)}><ShareIcon className="h-5 w-5" /></button> : null}
          {file.isTrashed ? <button className="grid h-9 w-9 place-items-center rounded-full text-drive-subtext transition hover:bg-slate-100 hover:text-drive-blue" onClick={() => onRestore(file)}><ArrowUturnLeftIcon className="h-5 w-5" /></button> : <button className="grid h-9 w-9 place-items-center rounded-full text-drive-subtext transition hover:bg-rose-50 hover:text-rose-600" onClick={() => onDelete(file)}><TrashIcon className="h-5 w-5" /></button>}
        </div>
      </div>
    </div>
  );
